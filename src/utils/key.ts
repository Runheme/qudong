...
export function getByteForCode(code: string) {
  ...
  } else if(isUserKeycode(code)) {
    return getByteForUserKeycode(code);
...
export function keycodeInMaster(keycode: string) {
  return keycode in basicKeyToByte || isLayerCode(keycode) || isUserKeycode(keycode);
}
...
export function isUserKeycode(keycode: string) {
  return keycode.startsWith("USER");
}
...
export function getByteForUserKeycode(keycode: string) {
  return parseInt(keycode.replace("USER", "")) + basicKeyToByte.USER00;
}
